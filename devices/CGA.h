...
private:
    ...
    // helper functions
    unsigned short xy_to_offset(int x, int y);  // computes the byte offset from CGA_START to the two bytes coding for (x, y) on screen
    void offset_to_xy(unsigned short offset, int& x, int& y);
