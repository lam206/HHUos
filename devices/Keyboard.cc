...
Key Keyboard::key_hit () {
    ...
    // poll keyboard controller until bit tells us we can read data
    do {
	    status = ctrl_port.inb();
    } while (!(status & Keyboard::outb));  // if outb is set this means we can collect from keyboard controller's output buffer
